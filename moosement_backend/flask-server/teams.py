...
@team_routes.route("/points", methods=["GET"])
def get_team_points():
...
@team_routes.route("/new_leaderboard", methods=["GET"])
def get_leaderboard():
    update_leaderboard()
...
@team_routes.route('/invite', methods=['POST'])
def invite_member():
    try:
        data = request.get_json()
        user_id = data.get("user_id")
...
@team_routes.route('/accept_invite/<invite_code>', methods=['POST'])
